...
#include "test/daa.h"
	...
	void CPUZ80Test::TestDAA()
	{
		...
		for (WORD i = 0; i < total; ++i)
		{
			...
			if (!TestNCHA(i, in_n, in_c, in_h, in_a, out_n, out_c, out_h, out_a, [&]() { DAA(); }))
			{
				++fail;
			}
		}
	...
	bool CPUZ80Test::TestNCHA(int num, bool in_n, bool in_c, bool in_h, BYTE in_a,
		...
		std::function<void()>func)
	{
		...
		{
			LogPrintf(LOG_ERROR, "TEST [%04d] FAILED: N[%d/%d]"
				", C[%d/%d]"
				", H[%d/%d]"
				", A[%02X/%02X]",
				num,
