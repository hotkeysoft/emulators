...
namespace events
{
	typedef std::map<SDL_Scancode, BYTE> KeyMap;
	...
	{
		LogPrintf(LOG_DEBUG, "InputKey: [%s] key: %d", evt.state == SDL_PRESSED ? "DOWN" : "UP", evt.keysym.scancode);
		KeyMap::const_iterator it = s_keyMap.find(evt.keysym.scancode);
		if (it != s_keyMap.end())
		{
			m_keyboard->InputKey(it->second | (evt.state == SDL_PRESSED ? 0 : 0x80));
		}
	}
}